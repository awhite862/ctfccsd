...
for nodes in [1, 2, 4, 8, 16, 32, 64]:
    ...
    script_file.write("#----------------------------------------------------\n")
    script_file.write("#SBATCH -J ctf_ccsd\n")
    script_file.write("#SBATCH -o ccsd.bench.ppn64.N%s.o%%j.out\n" % nodes)
    script_file.write("#SBATCH -e ccsd.bench.ppn64.N%s.o%%j.err\n" % nodes)
    script_file.write("#SBATCH -p normal\n")
    script_file.write("#SBATCH -N %s\n" % nodes)
    script_file.write("#SBATCH -n %s\n" % cores)
    script_file.write("#SBATCH -t 01:00:00\n")
    ...
    script_file.write("\n")
    script_file.write("\n")
    script_file.write("for no in %s\n" % no)
    script_file.write("do\n")
    script_file.write("  for nv in %s\n" % nv)
    script_file.write("  do\n")
